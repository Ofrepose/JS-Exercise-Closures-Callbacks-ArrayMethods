...
function processContains(item, list, callback) {
  // return callback(list.indexOf((item,a) => ? >= 0);
...
function getFullNames(runners) {
  const fullNames = [];
  for(const x in runners){
    fullNames.push(`${runners[x].last_name}, ${runners[x].first_name}`);
  }
  return fullNames;
}
...
function getRunnersByTShirtSize(runners,tShirtSize) {
  return runners.filter((runnerData) => runnerData.shirt_size == tShirtSize);
}
